...
export const useFeed = (
  ...
  const loadFeed = useCallback(async (reset = false) => {
    try {
      ...
      const data = await postService.getFeed(currentPage, 10, hashtag, category, hashtagId);
  ...
  }, [setPosts, setLoading, hashtag, category, hashtagId]);
  ...
  useEffect(() => {
    // Initial load
    loadFeed(true);
  }, [hashtag, category, hashtagId, loadFeed]);
