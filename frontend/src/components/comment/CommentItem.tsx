...
const CommentItem: React.FC<CommentItemProps> = ({
  ...
  // Defensive: some API responses may omit `likes`; ensure we have an array
  const likes = comment.likes ?? [];
  const [isLiked, setIsLiked] = useState(user ? likes.includes(userId) : false);
  const [likesCount, setLikesCount] = useState(likes.length);

  // `comment.userId` may be populated as an object or a string depending on the API
  const commentUserId =
