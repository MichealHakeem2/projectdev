...
export default function PromotionForm({ postId, postContent, businessId, onSuccess, onCancel }: PromotionFormProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        setCategories(data);
        if (data.length > 0) setTargetAudience(data[0]._id);
      } catch (err) {
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-3">
                {categories.map(cat => (
                  <button
                    key={cat._id}
                    onClick={() => setTargetAudience(cat._id)}
                    className={`flex items-center justify-between p-4 rounded-xl border-2 transition-all ${targetAudience === cat._id
                      ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/10'
                      : 'border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700'
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      <Target className={`w-5 h-5 ${targetAudience === cat._id ? 'text-primary-600' : 'text-gray-400'}`} />
                      <span className={`font-medium ${targetAudience === cat._id ? 'text-primary-900 dark:text-primary-100' : 'text-gray-600 dark:text-gray-400'}`}>
                        {cat.name}
                      </span>
                    </div>
                    {targetAudience === cat._id && <Check className="w-5 h-5 text-primary-600" />}
                  </button>
                ))}
              </div>
              ...
              <Card className="p-4 bg-gray-50 dark:bg-gray-900/50 border-none space-y-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900 dark:text-gray-100 font-bold">
                    {categories.find(c => c._id === targetAudience)?.name || targetAudience}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
